...
|Objectif 4+|Vérifier qu'un administrateur peut désactiver un utilisateur.
5+|
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|Desactivation_Utilisateur.1 ^|Accéder à la liste des utilisateurs et sélectionner un utilisateur à désactiver. ^|L'utilisateur est sélectionné pour désactivation. ^|L'utilisateur existe dans la base de données. ^|OK
^|Desactivation_Utilisateur.2 ^|Confirmer la désactivation. ^|L'utilisateur est marqué comme inactif et n'apparaît plus dans la liste des utilisateurs actifs. ^|Un utilisateur valide est sélectionné. ^|OK
5+|
5+|Commentaire :
Ce cas de test valide la capacité d'un administrateur à désactiver un utilisateur du système, rendant l'utilisateur inactif sans le supprimer de la base de données.
|====
...
^|Creation_Categorie.2 ^|Cliquer sur "Ajouter". ^|Le formulaire d'ajout de catégorie est affiché. ^|Aucune ^|OK
...
^|Ajout_Avis.4 ^|Soumettre l'avis. ^|L'avis est envoyé et associés au produit, visible pour les autres utilisateurs. ^|L'utilisateur doit avoir un produit disponible pour laisser un avis. ^|OK
5+|
5+|Commentaire :
